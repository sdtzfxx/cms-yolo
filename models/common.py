...
class Concat_att(nn.Module):
    def __init__(self, channel, dimension=1):
        ...
        self.d = dimension
        self.att = BiLevelRoutingAttention(channel)
    def forward(self, x):
...
class Conv_att1(nn.Module):
    # Standard convolution
    def __init__(self, c1, c2, k=1, s=1, p=None, g=1, act=True):  # ch_in, ch_out, kernel, stride, padding, groups
        super(Conv_att1, self).__init__()
        self.conv = nn.Conv2d(c1, c2, k, s, autopad(k, p), groups=g, bias=False)
        self.bn = nn.BatchNorm2d(c2)
        self.act = nn.SiLU() if act is True else (act if isinstance(act, nn.Module) else nn.Identity())
...
class Conv_att2(nn.Module):
    # Standard convolution
    def __init__(self, c1, c2, k=1, s=1, p=None, g=1, act=True):  # ch_in, ch_out, kernel, stride, padding, groups
        super(Conv_att2, self).__init__()
        self.conv = nn.Conv2d(c1, c2, k, s, autopad(k, p), groups=g, bias=False)
        self.bn = nn.BatchNorm2d(c2)
        self.act = nn.SiLU() if act is True else (act if isinstance(act, nn.Module) else nn.Identity())
        self.att = ACmix(c2)
...
class SPPCSPC(nn.Module):
    ...
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5, k=(5, 9, 13)):
        ...
        self.cv3 = RFAConv(c_, c_, 3, 1)
        self.cv4 = RFAConv(c_, c_, 1, 1)
        self.m = nn.ModuleList([nn.MaxPool2d(kernel_size=x, stride=1, padding=x // 2) for x in k])
        self.cv5 = RFAConv(4 * c_, c_, 1, 1)
        self.cv6 = RFAConv(c_, c_, 3, 1)
...
class autoShape(nn.Module):
    ...
    def forward(self, imgs, size=640, augment=False, profile=False):
        ...
        if isinstance(imgs, torch.Tensor):  # torch
            with torch.amp.autocast('cuda', enabled=p.device.type != 'cpu'):
...
class RFAConv_7(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=1, stride=1, g=1, dilation=1):
        super().__init__()
        self.conv = Conv(in_channels, out_channels, k=1)

        self.RFAConv_ = RFAConv(out_channels)

        self.bn = nn.BatchNorm2d(out_channels)

        self.relu = nn.ReLUELU()
...
class ADown(nn.Module):
    ...
    def __init__(self, c1, c2):
        ...
        super().__init__()
        self.c = c2 // 2
        self.cv1 = ODConv2d(c1 // 2, self.c, 1, 1, 1)
        self.cv2 = ODConv2d(c1 // 2, self.c, 1, 1, 0)
        self.cv3 = ODConv2d(c1 // 2, self.c, 3, 2,0)
